...
@media(max-width:480px) {
  ...
  .rkz-nav-hamburger.open span:nth-child(1) { transform: translateY(6.5px) rotate(45deg); background: #C9A84C; }
  .rkz-nav-hamburger.open span:nth-child(2) { opacity: 0; transform: scaleX(0); }
  .rkz-nav-hamburger.open span:nth-child(3) { transform: translateY(-6.5px) rotate(-45deg); background: #C9A84C; }
  .rkz-drawer {
  ...
  .rkz-drawer.open { transform: translateX(0); pointer-events: all; }
  .rkz-drawer-brand {
  ...
  .rkz-drawer.open .rkz-drawer-brand { opacity: 1; transform: translateY(0); }
  .rkz-drawer-brand em { color: #C9A84C; font-style: normal; }
  .rkz-dlink {
  ...
  .rkz-drawer.open .rkz-dlink { opacity: 1; transform: translateY(0); }
  .rkz-drawer.open .rkz-dlink:nth-of-type(1) { transition-delay: .12s; }
  .rkz-drawer.open .rkz-dlink:nth-of-type(2) { transition-delay: .17s; }
  .rkz-drawer.open .rkz-dlink:nth-of-type(3) { transition-delay: .22s; }
  .rkz-drawer.open .rkz-dlink:nth-of-type(4) { transition-delay: .27s; }
  .rkz-drawer.open .rkz-dlink:nth-of-type(5) { transition-delay: .32s; }
  .rkz-dlink:hover { color: #C9A84C; }
  .rkz-drawer-div { width: 40px; height: 1px; background: rgba(201,168,76,.25); margin: 18px 0; opacity: 0; transition: opacity .4s .34s; }
  .rkz-drawer.open .rkz-drawer-div { opacity: 1; }
  .rkz-dcta {
  ...
  .rkz-drawer.open .rkz-dcta { opacity: 1; transform: translateY(0); }
  .rkz-dcta:hover { background: #C9A84C; color: #05111e; }
  @media (max-width: 900px) {
    .rkz-nav-links { display: none; }
    .rkz-nav-cta { display: none !important; }
    .rkz-nav-hamburger { display: flex; }
  }
`;
...
// ── MAIN PAGE ─────────────────────────────────────────────────
export default function RAKEZPage({ onBack, onNavigate }) {
  const [_rkzOpen, setrkzOpen] = useState(false);
  // Lock body scroll when drawer open
  useEffect(() => {
    document.body.style.overflow = _rkzOpen ? "hidden" : "";
  ...
  return (
    <div className="rkz-root">
      ...
      <nav className={`rkz-nav${scrolled ? " scrolled" : ""}`}>
        <div className="rkz-nav-logo" onClick={()=>{if(onNavigate){onNavigate("home");window.scrollTo(0,0);}}}>INCO<em>ZONE</em></div>
        <ul className="rkz-nav-links">{["Services","Free Zones","About","Blog","Contact"].map(l=>{const pgMap={"Services":"services","Free Zones":"home","About":"about","Blog":"blog","Contact":"contact"};return <li key={l}><a href="#" onClick={e=>{e.preventDefault();if(onNavigate){onNavigate(pgMap[l]);window.scrollTo(0,0);}}}>{ l}</a></li>;})}</ul>
        <button className="rkz-nav-cta" onClick={()=>{if(onNavigate){onNavigate("schedule");window.scrollTo(0,0);}}}>Schedule Consultation</button>
      
        {/* Hamburger */}
        <button
          className={`rkz-nav-hamburger${_rkzOpen ? " open" : ""}`}
          onClick={() => setrkzOpen(o => !o)}
      ...
      {/* Mobile drawer */}
      <div className={`rkz-drawer${_rkzOpen ? " open" : ""}`}>
        <div className="rkz-drawer-brand"
          onClick={() => { setrkzOpen(false); if(onNavigate) { onNavigate("home"); window.scrollTo(0,0); } }}>
          INCO<em>ZONE</em>
        </div>
        {["Services","Free Zones","About","Blog","Contact"].map((l) => {
          ...
          return (
            <button key={l} className="rkz-dlink"
              onClick={() => { setrkzOpen(false); if(onNavigate) { onNavigate(pm[l]); window.scrollTo(0,0); } }}>
              {l}
            </button>
          );
        })}
        <div className="rkz-drawer-div" />
        <button className="rkz-dcta"
          onClick={() => { setrkzOpen(false); if(onNavigate) { onNavigate("schedule"); window.scrollTo(0,0); } }}>
      ...
      {/* ── WA FLOAT ── */}
      <div className="rkz-wa" onClick={()=>window.open("https://wa.me/971565834586","_blank")}>
