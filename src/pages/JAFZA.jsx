...
@media(max-width:480px) {
  ...
  .jfz-nav-hamburger.open span:nth-child(1) { transform: translateY(6.5px) rotate(45deg); background: #C9A84C; }
  .jfz-nav-hamburger.open span:nth-child(2) { opacity: 0; transform: scaleX(0); }
  .jfz-nav-hamburger.open span:nth-child(3) { transform: translateY(-6.5px) rotate(-45deg); background: #C9A84C; }
  .jfz-drawer {
  ...
  .jfz-drawer.open { transform: translateX(0); pointer-events: all; }
  .jfz-drawer-brand {
  ...
  .jfz-drawer.open .jfz-drawer-brand { opacity: 1; transform: translateY(0); }
  .jfz-drawer-brand em { color: #C9A84C; font-style: normal; }
  .jfz-dlink {
  ...
  .jfz-drawer.open .jfz-dlink { opacity: 1; transform: translateY(0); }
  .jfz-drawer.open .jfz-dlink:nth-of-type(1) { transition-delay: .12s; }
  .jfz-drawer.open .jfz-dlink:nth-of-type(2) { transition-delay: .17s; }
  .jfz-drawer.open .jfz-dlink:nth-of-type(3) { transition-delay: .22s; }
  .jfz-drawer.open .jfz-dlink:nth-of-type(4) { transition-delay: .27s; }
  .jfz-drawer.open .jfz-dlink:nth-of-type(5) { transition-delay: .32s; }
  .jfz-dlink:hover { color: #C9A84C; }
  .jfz-drawer-div { width: 40px; height: 1px; background: rgba(201,168,76,.25); margin: 18px 0; opacity: 0; transition: opacity .4s .34s; }
  .jfz-drawer.open .jfz-drawer-div { opacity: 1; }
  .jfz-dcta {
  ...
  .jfz-drawer.open .jfz-dcta { opacity: 1; transform: translateY(0); }
  .jfz-dcta:hover { background: #C9A84C; color: #05111e; }
  @media (max-width: 900px) {
    .jfz-nav-links { display: none; }
    .jfz-nav-cta { display: none !important; }
    .jfz-nav-hamburger { display: flex; }
  }
`;
...
// ── MAIN PAGE ──────────────────────────────────────────────────
export default function JAFZAPage({ onBack, onNavigate }) {
  const [_jfzOpen, setjfzOpen] = useState(false);
  // Lock body scroll when drawer open
  useEffect(() => {
    document.body.style.overflow = _jfzOpen ? "hidden" : "";
  ...
  return (
    <div className="jfz-root">
      ...
      <nav className={`jfz-nav${scrolled ? " scrolled" : ""}`}>
        <div className="jfz-nav-logo" onClick={()=>{if(onNavigate){onNavigate("home");window.scrollTo(0,0);}}}>INCO<em>ZONE</em></div>
        <ul className="jfz-nav-links">{["Services","Free Zones","About","Blog","Contact"].map(l=>{const pgMap={"Services":"services","Free Zones":"home","About":"about","Blog":"blog","Contact":"contact"};return <li key={l}><a href="#" onClick={e=>{e.preventDefault();if(onNavigate){onNavigate(pgMap[l]);window.scrollTo(0,0);}}}>{ l}</a></li>;})}</ul>
        <button className="jfz-nav-cta" onClick={()=>{if(onNavigate){onNavigate("schedule");window.scrollTo(0,0);}}}>Schedule Consultation</button>
      
        {/* Hamburger */}
        <button
          className={`jfz-nav-hamburger${_jfzOpen ? " open" : ""}`}
          onClick={() => setjfzOpen(o => !o)}
      ...
      {/* Mobile drawer */}
      <div className={`jfz-drawer${_jfzOpen ? " open" : ""}`}>
        <div className="jfz-drawer-brand"
          onClick={() => { setjfzOpen(false); if(onNavigate) { onNavigate("home"); window.scrollTo(0,0); } }}>
          INCO<em>ZONE</em>
        </div>
        {["Services","Free Zones","About","Blog","Contact"].map((l) => {
          ...
          return (
            <button key={l} className="jfz-dlink"
              onClick={() => { setjfzOpen(false); if(onNavigate) { onNavigate(pm[l]); window.scrollTo(0,0); } }}>
              {l}
            </button>
          );
        })}
        <div className="jfz-drawer-div" />
        <button className="jfz-dcta"
          onClick={() => { setjfzOpen(false); if(onNavigate) { onNavigate("schedule"); window.scrollTo(0,0); } }}>
      ...
      {/* ── WA FLOAT ── */}
      <div className="jfz-wa" onClick={()=>window.open("https://wa.me/971565834586","_blank")}>
